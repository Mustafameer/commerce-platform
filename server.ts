...
async function uploadAndCompressImageLocally(base64Data: string, filename: string): Promise<string> {
  try {
    ...
    if (!isPersistentFileStorageAvailable()) {
      ...
      console.log(`✅ Image compressed and stored inline for production (${sizeKB}KB)`);
      return dataUrl;
    }
...
async function testConnection() {
  try {
    ...
    console.log("🔌 Using connection string:", databaseUrl.substring(0, 50) + "...");
...
async function seedData() {
  try {
    ...
    if (adminCheck.rows.length === 0) {
      ...
      console.log("✅ Admin user created: phone: admin, password: admin");
    }
  ...
  try {
    ...
    app.post("/api/clear-all", async (req, res) => {
      try {
        ...
        console.error("��� في حذف البيانات:", error);
        res.status(500).json({ error: (error as any).message });
      }
    });
      ...
      try {
        ...
        res.json({
          ...
          message: canProceed 
            ...
            : `اعتذر: الرصيد المتاح ${availableCredit} ��� �� ������ ������� ${amount}`
        });
      } catch (error) {
        res.status(500).json({ error: (error as any).message });
      }
    ...
    app.post("/api/topup/purchase", async (req, res) => {
      try {
        ...
        if (availableCredit < total_amount) {
          return res.status(403).json({ 
            error: `������ ������ ${availableCredit} ��� �� ������ ������� ${total_amount}` 
          });
        }
      ...
      try {
        ...
        if (image_url.startsWith('data:')) {
          try {
            ...
            console.log(`✅ Product image stored: ${savedFileName || 'inline-data-url'}`);
          } catch (e) {
