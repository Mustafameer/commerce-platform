...
async function rebuildDatabase() {
  try {
    console.log('🚀 بدء إعادة بناء قاعدة البيانات محلياً...\n');

    // 1️⃣ حذف جميع الجداول
    ...
    await pool.query(`
      ...
      CREATE INDEX idx_topup_product_categories_store_id ON topup_product_categories(store_id);
      CREATE INDEX idx_topup_products_store_id ON topup_products(store_id);
      CREATE INDEX idx_topup_products_company_id ON topup_products(company_id);
      CREATE INDEX idx_topup_product_images_product_id ON topup_product_images(topup_product_id);
      CREATE INDEX idx_topup_product_images_created ON topup_product_images(created_at);
      CREATE INDEX idx_orders_topup_customer_id ON orders(topup_customer_id);
      CREATE INDEX idx_orders_customer_id_topup ON orders(customer_id);
    `);
    ...
    const store13 = await pool.query(`
      ...
      RETURNING id
    `);
    console.log('✅ تم إنشاء Store 13 (علي الهادي)');

    // Companies for Store 13
    const companies = await pool.query(`
      ...
      RETURNING id, name
    `);
    console.log(`✅ تم إنشاء ${companies.rows.length} شركات\n`);

    // Topup Products for each company
    console.log('📦 إضافة منتجات الشحن...');
    ...
    const products = await pool.query(`
      ...
      RETURNING id
    `, [companyIds[0], companyIds[1], companyIds[2]]);
    console.log(`✅ تم إنشاء ${products.rows.length} منتج\n`);

    // 5️⃣ إضافة صور العينة
    console.log('🖼️  إضافة صور الشحن...');
    ...
    for (let i = 0; i < products.rows.length; i++) {
      await pool.query(`
        ...
        VALUES ($1, $2, $3)
      `, [products.rows[i].id, images[i % 3], 'svg']);
    }
    console.log('✅ تم إضافة الصور\n');

    // 6️⃣ التحقق من البيانات
    console.log('✅ التحقق من البيانات:\n');
    const storeCount = await pool.query('SELECT COUNT(*) as count FROM stores');
    const companyCount = await pool.query('SELECT COUNT(*) as count FROM topup_companies WHERE store_id = 13');
    const productCount = await pool.query('SELECT COUNT(*) as count FROM topup_products WHERE store_id = 13');
    const imageCount = await pool.query('SELECT COUNT(*) as count FROM topup_product_images');
    
    console.log(`   📊 المتاجر: ${storeCount.rows[0].count}`);
    console.log(`   🏢 شركات الشحن: ${companyCount.rows[0].count}`);
    console.log(`   📦 منتجات الشحن: ${productCount.rows[0].count}`);
    console.log(`   🖼️  صور الشحن: ${imageCount.rows[0].count}\n`);

    console.log('🎉 تم بناء قاعدة البيانات بنجاح!\n');
    console.log('الآن يمكنك:');
    console.log('1. اختبار الـ endpoints محلياً');
    console.log('2. ثم تصدير قاعدة البيانات وتحديثها على Railway\n');
